...
func _ready() -> void:
	...
	# --- Zufällige Animationsgeschwindigkeit pro NPC ---
	var random_fps := randf_range(4.0, 6.5)
	var base_fps := 5.0
	animated_sprite.speed_scale = random_fps / base_fps
	...
	if npc_data.can_sit:
		...
		return   # >>> ALLES Weitere (Idle/Patrol etc.) NICHT initialisieren
...
func _physics_process(delta: float) -> void:
	...
	# Sitzen blockiert jede Bewegung
	if npc_data.can_sit:
		return
...
func _set_facing(dir: String) -> void:
	...
	if anim_to_play != "" and animated_sprite.sprite_frames.has_animation(anim_to_play):
		...
		if anims.size() > 0:
			...
			push_warning(
				"NPC '%s': Animation '%s' nicht gefunden, spiele '%s'"
				% [name, anim_to_play, anims[0]]
			)
